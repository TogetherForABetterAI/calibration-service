...
def test_integration_main_runs_without_errors(mock_global_config):
    ...
    fake_listener_middleware = FakeMiddleware(config=mock_global_config.middleware_config, messages={CONNECTION_QUEUE_NAME: notification}, queue=middleware_queue)
...
def test_exceed_upper_bound_clients(mock_global_config):
    ...
    notifications = []
    for i in range(mock_global_config.server_config.upper_bound_clients + 3):
        notifications.append(f'{{"client_id": "client-{i:03d}"}}'.encode('utf-8'))
        
    middleware_queue = Queue()
    
    fake_listener_middleware = FakeMiddleware(config=mock_global_config.middleware_config, messages={CONNECTION_QUEUE_NAME: notifications}, queue=middleware_queue)

    initialize_logging(mock_global_config.log_level.upper())

    def middleware_factory_fake(*args, **kwarg):
        return FakeMiddleware(config=mock_global_config.middleware_config, queue=None, messages={}, processing_delay=1)
    ...
    assert len(fake_listener_middleware.msg_ack) == mock_global_config.server_config.upper_bound_clients + 3
...
# def test_exceed_upper_bound_clients_and_continue_handling_connections(mock_global_config):
#     """Prueba de integración para verificar que el servidor maneje el límite superior de clientes."""
...
#     assert fake_listener_middleware.clients_handled_after_stop_consuming == 3
